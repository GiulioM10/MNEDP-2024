...
if(~exist('mybbtr30.m'))
     addpath('../bbtr30')
     addpath('../MNEDP_MG')
     disp('../libraries added to the path')
end
...
utrue = @(x, y) -tanh(160*x - 160);

gradientutrue = @(x, y) [160*tanh(160*x - 160)^2 - 160;
                         0];
 
laplacianutrue = @(x,y) -320*tanh(160*x - 160)*(160*tanh(160*x - 160)^2 - 160);
...
f = @(x, y) - mu(x, y) * laplacianutrue(x, y) + sigma(x, y) * utrue(x, y);

gD_f = @(x, y) -tanh(160*x - 160);
...
[A, b] = assembleSystem(K, geom, mu, beta, sigma, f, gD_f, gN_f, false, true);
