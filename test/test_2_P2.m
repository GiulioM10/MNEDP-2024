...
dVertices = [ 0 0
              ...
              0 1];
dBoundary = 1:6;
bcBoundary = [9 11 13 15 17 2];
bcVertices = [1 3 5 7 21 23];
bcValues = [3.0, 2.0];
checkArea = "Y";
checkAngle = "N";
areaValue = 0.001;
...
utrue = @(x, y) (1000*x.*y.*(0.5-x).*(y-0.5).*(1-x).*(1-y)+(x+y)).*(~((x>0.5&x<=1)&(y>0.5&y<=1)));

gradientutrue = @(x, y) [1 - 1000*x*y*(x - 1)*(y - 1)*(y - 1/2) - 1000*x*y*(x - 1/2)*(y - 1)*(y - 1/2) - 1000*y*(x - 1)*(x - 1/2)*(y - 1)*(y - 1/2);
                         1 - 1000*x*y*(x - 1)*(x - 1/2)*(y - 1) - 1000*x*y*(x - 1)*(x - 1/2)*(y - 1/2) - 1000*x*(x - 1)*(x - 1/2)*(y - 1)*(y - 1/2)];
...
f = @(x, y) -laplacianutrue(x, y) + beta(x, y)'*gradientutrue(x,y) + sigma(x, y)*utrue(x, y);
